...
Период планового закрытия сегментов — 900 секунд. Первый может быть короче,
ограничение размера может закрыть сегмент раньше, а текущий сбор — задержать
закрытие. Веб-сервер читает новые данные из `active.wal`, не дожидаясь готового
сегмента. `Ctrl+C` останавливает сбор и сохраняет журнал; для продолжения
...
процессов один. Каждый процесс собирает доступные базы своего сервера;
...
| --- | --- |
| `KRONIKA_COLLECTOR_MODE` | По умолчанию `local`: Linux и настроенный локальный PostgreSQL. `postgresql`: только PostgreSQL, без чтения метрик Linux, процессов и cgroup машины сборщика. |
