...
test("dense paging resets, ignores stale work, and preserves retry state", async () => {
  ...
  assert.match(source, /const requestOrder = visibleSource === "processes"[\s\S]*const snapshotTarget = [\s\S]*snapshotTargetKey\(snapshotGroups, cursor, requestOrder, denseOptions\)/)
  ...
  assert.match(source, /const refreshReady = !loading && !neighborPending && cursorState !== "loading" && densePageState !== "loading"/)
