...
| `/api/snapshot/neighbor` | `GET` | Найти время предыдущего или следующего снимка указанных секций. Параметры описаны ниже. |
...
Поиск соседнего снимка принимает `section` (можно повторить до 32 раз), `at`
и `direction=next|previous`. Необязательные `from` и `to` ограничивают поиск;
обе границы включены. Время задаётся в микросекундах Unix. Результат — ближайший
записанный снимок со сдвигом не меньше секунды в выбранную сторону, в том числе
из другого сегмента:
...
Если подходящего снимка нет, оба значения равны `null`. Ответ содержит
`Cache-Control: private,no-store`, без ETag: после пустого результата повторный
запрос сможет найти дописанные данные. Кеширование самих завершённых снимков
сохраняется. HTML-отчёт выполняет тот же поиск в пределах экспортированного времени.

Для загрузки экрана используется `selection=latest`: выбирается последний снимок
не позже `at`, в том числе из более раннего сегмента с пересекающимся временем.
Режим по умолчанию `selection=anchor` сохраняет приоритет явно указанного сегмента.
Запрос конкретной строки остаётся привязанным к этой строке. Разные URL и ETag
не дают браузеру использовать кеш одного режима для другого.
