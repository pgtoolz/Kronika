...
// Private self-exec mode, handled before normal collector configuration and WAL startup.
const HELPER_ARGUMENT: &str = "--kronika-statvfs-helper";
// Prefer missing capacity data to waiting longer for these optional probes.
// One waiting budget covers all mounts, measured from collect() entry. Expiry
// stops polling and requests termination; setup and cleanup have no hard timeout.
const HELPER_WAIT_BUDGET: Duration = Duration::from_secs(1);
// Short polls let fast passes finish well before the waiting budget expires.
// Cap the last sleep at the deadline; never use a blocking child.wait().
const POLL_INTERVAL: Duration = Duration::from_millis(5);
// Response: status byte (0 = unavailable, 1 = present), then four little-endian
// i64s: total bytes, available bytes, total inodes, available inodes. Status 0
// uses zeroed fields. Fixed lengths let the parent discard an incomplete tail.
const RECORD_LEN: usize = 1 + 8 + 8 + 8 + 8;
// Keep a child until try_wait confirms exit: kill need not take effect immediately.
// Skip later passes while it remains alive instead of accumulating helpers.
// The mutex also prevents concurrent passes from spawning separate helpers.
...
///
/// Input is a sequence of NUL-terminated UTF-8 mount paths; output has one
/// fixed-size record per path, in the same order. Probes run sequentially: a
/// stalled mount also prevents later mounts from being measured in this pass.
/// Flush each result so it survives termination during the next probe.
///
/// # Errors
///
...
/// Collect capacity for the allowed filesystem types.
///
...
pub(crate) fn collect(entries: &[MountEntry]) -> Vec<Option<FsSpace>> {
    ...
    let deadline = started + HELPER_WAIT_BUDGET;
    let eligible: Vec<(usize, &str)> = entries
        .iter()
        .enumerate()
        .filter(|(_, entry)| is_supported_filesystem(&entry.fstype))
...
fn collect_eligible(
    ...
    // Anonymous files avoid pipe backpressure while the parent only polls.
    // The response can be read without waiting for EOF, even if the child has
    // not exited after termination was requested.
...
fn terminate_helper(mut child: Child) -> (Option<String>, Option<String>, bool, Option<Child>) {
    // Never follow kill with a blocking wait: that would reintroduce the stall
    // this helper is meant to isolate. Hand an unreaped child back to the slot.
...
fn read_complete_records(response: &File, expected: usize) -> std::io::Result<Vec<u8>> {
    // Snapshot the complete prefix after polling stops. Records carry no
    // timestamps, so this can include a result written just after the deadline.
    ...
    // The child's stdout shares the file offset; read from an explicit offset.
...
fn log_helper_outcome(outcome: HelperOutcome, eligible: usize, completed: usize) {
    match outcome {
        ...
        HelperOutcome::TimedOut {
            ...
            &[
                field("deadline_ms", HELPER_WAIT_BUDGET.as_millis()),
                field("eligible", eligible),
                field("completed", completed),
...
// Select filesystem implementations, not the transport or health of their storage.
fn is_supported_filesystem(fstype: &str) -> bool {
    matches!(
        fstype,
...
#[cfg(test)]
#[path = "tests/filesystem_capacity.rs"]
mod tests;
