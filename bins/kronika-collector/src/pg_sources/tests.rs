...
async fn enumeration_permission_error_still_admits_activity_and_locks() {
    ...
    sources
        .collect(
            &crate::scheduler::DueSet::for_test(vec![crate::scheduler::SourceKind::Pg]),
            &mut |event| observations.push(event),
            |batch, _settings| {
                match batch {
                    super::PgBatch::Activity(_, rows) => admitted.push(("activity", rows.len())),
                    super::PgBatch::Locks(_, rows) => admitted.push(("locks", rows.len())),
                    _ => {}
                }
                Ok::<_, ()>(BatchWrite::default())
            },
...
fn serve_enumeration_denied(listener: &TcpListener) {
        ...
        match tag[0] {
            ...
            b'Q' => {
                ...
                if query == SERVER_PROBE_SQL {
                    let columns = ["version", "oid", "database", "role_oid", "role", "visible"]
                        .into_iter()
...
fn write_protocol_row(stream: &mut TcpStream, columns: &[ProtocolColumn], binary: bool) {
