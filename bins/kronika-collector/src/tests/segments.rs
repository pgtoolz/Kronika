...
fn user_window(segment: &mut SegmentState, ts: i64) -> (FlushedPart, Vec<(u8, u32)>) {
    let users = &mut segment.user_names;
    users.observe_user(0, 26);
    users.observe_user(0, 26);
    let (rows, pending) = users.prepare_rows(&mut segment.interner, ts);
...
fn test_config(storage_dir: &Path) -> Config {
    Config {
        ...
        prometheus_metrics: Vec::new(),
        prometheus_preset: kronika_prometheus::catalog::DEFAULT_PRESET.to_owned(),
...
fn journal_full_writes_accumulated_segment_and_defers_window() {
    ...
    assert_eq!(segment.first_id.map(SegmentId::get), None);
}
...
fn invalid_part_at_journal_cap_is_transactional() {
    ...
    .expect("append first");
    let pending_id = segment
        .interner
        .intern(b"pending-after-first-frame")
        .expect("intern an uncommitted value");
    let bytes_before = fs::read(&path).expect("snapshot active.wal");
    let first_before = segment.first_id.map(SegmentId::get);
    ...
    assert_eq!(segment.first_id.map(SegmentId::get), first_before);
    assert_eq!(segment.interner.stats(), dictionary_before);
    assert_eq!(
        segment.interner.window().resolve(pending_id),
        Some(Resolved::Str(b"pending-after-first-frame")),
        "a rejected append must retain pending dictionary bytes"
    );
...
fn user_reference_survives_recovery_and_is_reemitted_after_forced_rollover() {
    ...
    let mut segment = SegmentState {
        user_names: crate::os_sources::SegmentUserNames::with_passwd(snapshot.clone()),
    ...
    segment.user_names.confirm_written(&pending);
    drop(journal);
    drop(owner);
    ...
    let mut segment = SegmentState {
        user_names: crate::os_sources::SegmentUserNames::with_passwd(snapshot),
        ..SegmentState::default()
    };
...
fn one_age_deadline_survives_later_appends_and_wall_clock_changes() {
    ...
    assert_eq!(
        crate::collector::timer_sleep_delay(now, 0, &sched, &segment, None),
        None
    );
