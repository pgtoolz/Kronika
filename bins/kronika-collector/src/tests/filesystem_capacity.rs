use super::{RECORD_LEN, decode_records, is_supported_filesystem, write_record};
use kronika_source_os::FsSpace;

#[test]
fn filesystem_allowlist_is_explicit() {
    for fstype in [
        "ext2", "ext3", "ext4", "xfs", "btrfs", "f2fs", "zfs", "tmpfs", "overlay",
    ] {
        assert!(is_supported_filesystem(fstype), "{fstype}");
    }
    for fstype in [
        "nfs",
        "nfs4",
