...
Эталонная сборка использует Ubuntu 24.04, Clang 18.1.3 и `llvm-ar-18`:
...
репозитория и каталога Cargo и начальное значение генератора `const-random`,
удаляет идентификатор компилятора C и отключает инкрементальную компиляцию
независимо от локальной настройки Cargo. Для сжатия
используется `pako` из зависимостей интерфейса с версией, закреплённой в lock-файле:
Apple gzip и GNU gzip дают разные байты.

Размер WebAssembly — 10 194 936 байт, gzip — 2 471 258 байт, SHA-256 gzip:
