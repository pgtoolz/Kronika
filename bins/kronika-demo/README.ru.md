...
Требования: Make и Docker с Compose v2 на машине с Linux для amd64 или arm64.
...
CPU ограничена отрезками по 100 мс; поток памяти использует один массив
фиксированного размера и раз в секунду обращается к каждой его странице. Сетевой
...
- CPU: 270 CPU-секунд в час, в среднем 7,5% одного ядра или 3,75% от двух ядер
  Compose.
- Память: 32 MiB анонимной памяти с регулярным обращением к каждой странице. Файл размером 8 MiB жёстко
