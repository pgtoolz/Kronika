...
Эти сервисы запускают сборщик и веб-сервер при загрузке системы и перезапускают
их после сбоя. Программы находятся в `/usr/local/bin`, хранилище принадлежит root,
веб-сервер слушает localhost. Для каждого каталога запускайте один сборщик
и один веб-сервер. Перед запуском сервиса остановите соответствующую программу,
если она уже работает в терминале.
...
Для каждого сервера PostgreSQL создайте отдельный сервис systemd, запускающий
`kronika-collector`. У каждого сервиса должны быть своё имя, свой файл настроек
с DSN сервера и отдельный каталог хранения. Все сервисы используют один бинарник.
Каждый веб-сервис читает один каталог хранения и требует своего адреса
прослушивания. См.
...
Права root позволяют читать защищённые счётчики процессов и журналы.
`UMask=0077` закрывает новые файлы от других пользователей. Веб-серверу нужна
запись в хранилище для индексов. Он может показывать записи после остановки
сборщика.
