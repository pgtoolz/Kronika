...
Данные ресурсов процесса доступны только при записанном признаке общего пространства процессов с Linux в том же сегменте. PostgreSQL-only, контейнеры и старые записи не связывают процессы по одному совпавшему PID. SQL-данные о ходе VACUUM и Activity остаются доступны.
