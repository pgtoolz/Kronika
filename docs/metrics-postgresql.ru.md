...
Активность, ожидания блокировок и ход VACUUM собираются с интервалом `--pg-activity-interval-s` — по умолчанию 10 секунд. После успешного чтения непустого снимка ожиданий интервал становится равным меньшему из двух значений: `--pg-activity-interval-s` и `--pg-activity-blocked-interval-s` (по умолчанию 5 секунд). Успешное чтение пустого снимка возвращает настроенный интервал; ошибка его не меняет. Запросы выполняются последовательно, поэтому медленный запрос может задержать следующий снимок. Поведение таймера и сигналов описано в [справочнике интервалов](metrics-time.ru.md).
...
Для статистики `pg_stat_statements`, `pg_store_plans` и их представлений info действует `--pg-statements-interval-s`: по умолчанию и не менее 300 секунд. Интервал отсчитывается после завершения предыдущего прохода PostgreSQL, включавшего эти источники. `SIGUSR2` не отменяет это ограничение.
