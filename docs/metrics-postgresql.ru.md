...
Активность, ожидания блокировок и ход VACUUM собираются с интервалом `KRONIKA_PG_ACTIVITY_INTERVAL_S` — по умолчанию 10 секунд. После успешного чтения непустого снимка ожиданий интервал становится равным меньшему из двух значений: `KRONIKA_PG_ACTIVITY_INTERVAL_S` и `KRONIKA_PG_ACTIVITY_BLOCKED_INTERVAL_S` (по умолчанию 5 секунд). Успешное чтение пустого снимка возвращает настроенный интервал; ошибка его не меняет. Запросы выполняются последовательно, поэтому медленный запрос может задержать следующий снимок. Поведение таймера и сигналов описано в [справочнике интервалов](metrics-time.ru.md).
...
Для числа плановых и запрошенных контрольных точек сначала используются `pg_stat_checkpointer.num_timed/num_requested`, а при их отсутствии — `pg_stat_bgwriter.checkpoints_timed/checkpoints_req`. Снимки подготовленных транзакций относятся к открытому интервалу между соседними метками снимков баз. Для хода VACUUM границами служат соседние метки Activity: эти источники собираются с одним интервалом. Поэтому повторные снимки одного работника не суммируются за более длинный интервал сбора баз. Пустой интервал даёт нулевое число подготовленных транзакций или работников autovacuum; возраст подготовленных транзакций остаётся `null`. Если снимков, задающих границы, нет, ноль не подставляется.
...
Для статистики `pg_stat_statements`, `pg_store_plans` и их представлений info действует `KRONIKA_PG_STATEMENTS_INTERVAL_S`: по умолчанию и не менее 300 секунд. Интервал отсчитывается после завершения предыдущего прохода PostgreSQL, включавшего эти источники. `SIGUSR2` не отменяет это ограничение.
...
| Table + TOAST / `displayed_storage_bytes` | Основной файл таблицы + TOAST; пользовательские индексы, карта свободного места (FSM) и карта видимости (VM) основной таблицы не входят |
...
Данные ресурсов процесса доступны только при записанном признаке общего пространства процессов с Linux в том же сегменте. При сборе только PostgreSQL, в контейнерах и старых записях совпадения PID недостаточно для этой связи. SQL-данные о ходе VACUUM и Activity остаются доступны.

Раздел **Process load** показывает затраты процесса за время наблюдаемого запуска. Для его PID выбираются снимки ОС не позже первой и последней меток запуска. `H` — записанное число учётных тактов CPU в секунду. Тогда время CPU в миллисекундах равно `1000Δ(utime+stime)/H`, где `utime` и `stime` — процессорные такты в пользовательском и системном режиме. Доля CPU равна `min(100, 100 × CPU seconds / OS sample elapsed seconds)`: процессорное время делится на время между выбранными снимками ОС, обе величины — в секундах. Время ожидания блочного ввода-вывода равно `1000Δblkdelay_ticks/H` мс.

Прочитанные и записанные байты, а также число страничных ошибок с чтением с диска равны `Δread_bytes`, `Δwrite_bytes`, `Δmajflt`. Доля прочитанных байтов — `min(100, 100Δread_bytes/(B × final heap_blks_scanned))`, где `final heap_blks_scanned` — число просмотренных блоков таблицы в последнем снимке запуска. Эти разности включают всю работу PID между выбранными снимками ОС. Если размер блока или число учётных тактов в секунду неизвестны, недоступны только вычисления с этим значением.
