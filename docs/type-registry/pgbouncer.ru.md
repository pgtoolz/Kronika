...
По подключениям, заданным через `--pgbouncer-dsn` или `KRONIKA_PGBOUNCER_DSNS`, сборщик узнаёт путь `logfile` через `SHOW CONFIG`. Пути и шаблоны, заданные через `--pgbouncer-log` или `KRONIKA_PGBOUNCER_LOGS`, добавляют файлы явно. Файлы должны быть доступны для чтения на машине сборщика и иметь формат, описанный ниже. Источник собирает события логов; `SHOW POOLS`, `SHOW STATS`, `SHOW CLIENTS` и строки `stats:` не собираются.
