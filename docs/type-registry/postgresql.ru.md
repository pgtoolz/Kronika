...
В режиме `local` сборщик использует подключение для метрик (`--pg-dsn` или `KRONIKA_PG_DSN`), чтобы найти файлы через `pg_current_logfile()`. Пути и шаблоны, заданные через `--pg-log` или `KRONIKA_PG_LOGS`, добавляются к найденным файлам. В режиме `postgresql` читаются только эти явно заданные пути. Читать их должен сам сборщик: файлы должны быть доступны на его машине. Файл, найденный обоими способами, читается один раз. Ранее не читанный файл открывается с начала; известный файл продолжает чтение с сохранённой позиции в байтах.
