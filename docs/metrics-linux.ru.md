...
только для согласованной группы. Память использует иерархические `total_*`,
независимо от порядка строк. Отсутствие ограничения не означает unlimited.
При положительных квоте `Q`, периоде `P` в микросекундах и числе разрешённых CPU
`S` доступно `min(Q/P, S)` ядра; без `S` — `Q/P`. Например,
