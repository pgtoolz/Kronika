...
| OS `scope` | К чему относятся данные: `0` — машина; `1` — pod; `2` — сетевое пространство имён pod; `3` — контейнер; `4` — неизвестно. Pod — группа контейнеров, которая может совместно использовать сеть. |
