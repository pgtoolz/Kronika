...
|---|---|
| `instance_metadata.environment` | Вид окружения сборщика: машина (`machine`) или контейнер (`container`). Виртуальная машина (VM) считается машиной. Поле выбирает основной источник ресурсов и PSI; обнаружение групп работает и на машине. |
...
В режиме `KRONIKA_COLLECTOR_MODE=postgresql` сбор Linux отключён. В режиме `local` сборщик записывает все видимые и доступные каталоги cgroup v2 на машине, VM и в контейнере. Нынешние показатели ресурсов контейнера относятся к самому верхнему доступному предку группы сборщика. См. [сбор cgroup](#container-cgroups).
...
При запуске и затем по умолчанию каждые 30 с сборщик обнаруживает каталоги и
...
Для сбора требуется cgroup v2. На системах только с v1 метрики cgroup и связи
процессов с группами недоступны; остальные включённые источники Linux и PostgreSQL
продолжают работать.
