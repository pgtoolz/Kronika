...
При запуске в режиме `local` контейнер проверяет наличие видимого монтирования
...
Машина по-прежнему получает PSI из `/proc/pressure`. Остальные включённые источники
и все прежние записи cgroup остаются доступны.
