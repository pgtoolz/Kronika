...
В режиме `KRONIKA_COLLECTOR_MODE=postgresql` сбор Linux отключён. В режиме `local` машина, VM и контейнер записывают все видимые и доступные каталоги cgroup v2. Нынешние показатели ресурсов контейнера относятся к самому верхнему доступному предку группы сборщика. См. [сбор cgroup](#container-cgroups).
...
| `os_cgroup_v2_cpu` | Общее/пользовательское/системное время и throttled в мкс; периоды и throttled-периоды; локальные quota/period и число CPU в effective cpuset |
| `os_cgroup_v2_memory` | Current/max/high и anon/file/kernel/slab в байтах; обычные события low/high/max/oom/oom_kill; отдельно local high/max/oom/oom_kill/oom_group_kill |
...
Строки записываются ограниченными порциями с общим временем наблюдения;
WAL и готовые сегменты сохраняют все порции. Нынешние показатели ресурсов используют
выбранный контекст, описанный ниже. Источник PSI один: системный на машине или
выбранный предок в контейнере. Доступные поля CPU `full` для cgroup сохраняются;
системный CPU `full` остаётся неопределённым.
...
Счётчики родителя включают его детей, в том числе sidecar-контейнеры, если это
общий pod. Они используются один раз, без добавления счётчиков детей. Видимый
