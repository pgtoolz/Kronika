...
| Показатели cgroup | Самый верхний доступный предок для каждого контроллера. Счётчики и доступные ресурсы относятся к этой записанной группе и её идентификатору. |
| Файловые системы | Видимые сборщику точки монтирования с данными. Их ёмкость читается вызовом `statvfs` по видимому пути. |

В режиме `KRONIKA_COLLECTOR_MODE=postgresql` сбор Linux отключён. В режиме `local` записи обычной машины/VM не содержат ресурсов cgroup. В контейнере сборщик поднимается от своей группы до самого верхнего видимого и читаемого предка в каждом подходящем монтировании. Соседние группы не обходятся; наличие процессов непосредственно в родителе не требуется. См. [группы ресурсов контейнера](#container-cgroups).
...
В v2 контроллеры относятся к одной выбранной группе. В v1 у каждого остаются
свой корень, путь и идентификатор. CPU из cpuacct сопоставляется с квотой и cpuset
только для согласованной группы. Память использует иерархические `total_*`, включая дочерние группы.
...
| Local memory max | v2 `memory.max`; v1 `memory.limit_in_bytes` | Байты; в новой структуре `max_unlimited` отдельно обозначает неограниченную память, отсутствие данных остаётся неизвестным |
