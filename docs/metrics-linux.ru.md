...
Cgroup — группа процессов Linux с общим учётом ресурсов и ограничениями. Контроллер отвечает за отдельный ресурс, например CPU или память.

| Записанный факт | Значение и использование |
|---|---|
| `instance_metadata.environment` | Вид окружения сборщика: машина (`machine`) или контейнер (`container`). Виртуальная машина (VM) считается машиной. Поле выбирает основной источник ресурсов и PSI. Обнаружение cgroup работает только в контейнерах. |
| OS `scope` | `0` — машина; `1` — pod в прежних записях; `2` — сетевое пространство pod; `3` — контейнер в прежних записях; `4` — группа без предположения о машине или pod. В новых записях cgroup указывается выбранный путь. |
...
| Показатели cgroup | Самый верхний доступный предок cgroup v2. Счётчики и доступные ресурсы относятся к этой записанной группе и её идентификатору. |
| Файловые системы | Видимые сборщику точки монтирования с данными. Их ёмкость читается вызовом `statvfs` по видимому пути. |

В режиме `--mode postgresql` сбор Linux отключён. В режиме `local` cgroup собираются только в контейнерах с видимым монтированием cgroup v2. На машинах и VM продолжается сбор остальных метрик ОС и процессов. Нынешние показатели ресурсов контейнера относятся к самому верхнему доступному предку группы сборщика. См. [сбор cgroup](#container-cgroups).
...
### Режимы таблицы и поля

| Режим — набор столбцов | Что показывает |
|---|---|
...
Сводка над таблицей относится ко всему снимку процессов, независимо от поиска и открытой страницы. General и Tree показывают число процессов, сумму потоков, число процессов в состоянии `R` и число PID из последнего снимка активности PostgreSQL не позже снимка процессов. CPU складывает ядра, занятые пользовательским кодом/ядром ОС, ожидание CPU в миллисекундах за секунду и добровольные/вынужденные переключения контекста за секунду. Memory складывает RSS, виртуальную память, память в подкачке и большие страничные промахи за секунду. Disk складывает байты и системные вызовы чтения/записи за секунду.

Число процессов PostgreSQL требует записанного признака общего пространства процессов в этом сегменте; без него число неизвестно. Это же условие действует для отметки PG и связей Activity/Vacuum с процессами OS. Старые записи не подтверждают общие процессы.
...
Общие графики **Device busy** и **Queue depth** показывают максимум среди устройств в каждый момент. Отдельные линии относятся к устройствам; полностью нулевые за час линии скрываются, если есть активное устройство. Дорожка Disk и ячейки Storage таблицы USE показывают занятость и очередь самого занятого устройства. При точном равенстве выбирается устройство ниже по записанной цепочке, иначе — меньшее major:minor. **Active I/O** в обзоре машины — сумма `Σio_in_progress`, а **Block devices** — число записанных строк устройств.
...
В поддерживаемых контейнерах при запуске и затем по умолчанию каждые 30 с сборщик обнаруживает каталоги и
читает их метрики. Обход начинается от видимых монтирований cgroup2, в том числе
вне `/sys/fs/cgroup`. Пустые, промежуточные и threaded-группы включены.
Повторные монтирования одной группы читаются один раз, переходов по симлинкам нет.
Скрытые пространства имён недоступны. Группа может возникнуть и исчезнуть между
проходами. Обход не атомарный: готовые порции уже читаются, пока собираются следующие.

| Секция | Записанные значения |
|---|---|
| `os_cgroup_v2_group` | Путь, идентификатор каталога, корень монтирования, точно известный родитель, флаги `memory_localevents` и `pids_localevents`; строка есть и без файлов ресурсов |
| `os_cgroup_v2_cpu` | Время CPU (общее, пользовательское, системное) и время ограничения квотой, мкс; число периодов и периодов с ограничением; локальная квота, её период и число CPU из `cpuset.cpus.effective` |
| `os_cgroup_v2_memory` | `current`, `max`, `high`, `anon`, `file`, `kernel`, `slab`, байты; события low/high/max/oom/oom_kill из `memory.events`; high/max/oom/oom_kill/oom_group_kill из `memory.events.local` |
| `os_cgroup_v2_pids` | Число потоков, локальный лимит, отказы `max`; источник событий: `0` неизвестен, `1` `pids.events.local`, `2` `pids.events` |
| `os_cgroup_v2_io` | Для каждой пары группа/устройство: `major:minor`, rbytes/wbytes/rios/wios |

Отсутствующие или некорректные значения остаются null. Значение `max` означает
отсутствие лимита и отличается от непрочитанного значения. Для обычных событий
памяти сохранён режим монтирования. Сам по себе источник `pids.events` не
определяет локальный ограничитель: его смысл зависит от ядра и режима монтирования.
Родительские счётчики могут включать детей, а устройства — нижележащие слои;
такие значения не складываются. Пересозданный каталог начинает отдельную историю.

Строки каждого прохода cgroup имеют общее время наблюдения.
Host → Cgroups показывает записанные группы в таблицах CPU, Memory, I/O и Tasks.
Поиск и сортировка применяются ко всем подходящим строкам до разделения на страницы.
Для каждого ресурса берётся последнее наблюдение не позже курсора. При равном
времени новые записи всех групп имеют приоритет перед прежними структурами.
Отсутствующие поля остаются недоступными. В I/O каждая пара группа/устройство
показана отдельно. Лимиты принадлежат самой группе; ограничения выбранного
предка не подставляются. История следует записанному идентификатору каталога:
пересозданный путь не продолжает прежние счётчики.
Доля периодов с ограничением равна `100 × Δnr_throttled / Δnr_periods`;
это не доля потерянного CPU. Время ограничения, локальные завершения по OOM и
отказы создания потоков показывают прирост счётчика за наблюдаемый интервал.
Без предыдущего значения, при уменьшении счётчика или смене идентификатора
каталога либо источника событий прирост недоступен.
Сводка контейнера и её графики показывают выбранного предка, описанного ниже.
Источник PSI — система на машине или выбранный предок в контейнере.
Доступные поля CPU `full` для cgroup сохраняются; системный CPU `full` остаётся неопределённым.

<a id="capacity-и-membership"></a>

### Доступные ресурсы и принадлежность группе

При запуске в контейнере в режиме `local` сборщик проверяет наличие видимого монтирования
cgroup v2. Подтверждённое отсутствие отключает метрики cgroup, связи процессов
с группами и контейнерный PSI до перезапуска. Ошибка чтения записывается в журнал,
попытки сбора продолжаются. На машине и в режиме `postgresql` этой проверки нет.
На машине PSI по-прежнему собирается из `/proc/pressure`. Остальные включённые источники продолжают работать.

`os_cgroup_context` хранит выбранные пути, корни монтирования, идентификаторы
контроллеров и доступные ограничения. Сборщик поднимается от своей группы до
самого верхнего доступного предка. Наличие отдельного файла метрик не влияет
на выбор: отсутствующий CPU, память или PSI остаётся неизвестным. Счётчики
дочерней группы сборщика и данные `/proc` машины не подставляются. Отказ доступа
к одному каталогу не исключает другого известного доступного предка. Чтение
ограничено видимым монтированием и пространством имён; скрытые родители недоступны.

Выбранная группа охватывает своих потомков, в том числе sidecar-контейнеры,
если это общий pod. Учёт потомков зависит от метрики. Счётчики родителя
используются один раз, без добавления счётчиков детей. Видимый
`/` обозначает границу монтирования или пространства имён, а не обязательно
узел Kubernetes или pod. Показываются фактические пути и идентификаторы групп.
Выбор предка не подтверждает общее пространство PID и не определяет контейнер PostgreSQL.

Контроллеры относятся к одной выбранной группе cgroup v2.
Непрочитанный лимит остаётся неизвестным; это не означает отсутствие ограничения.
При положительных квоте `Q`, периоде `P` в микросекундах и числе разрешённых CPU
`S` записанный лимит CPU в ядрах равен `min(Q/P, S)`; без `S` — `Q/P`. Например,
`150000/100000 = 1.5` ядра. В новых записях контекста достаточно любого из
этих конечных ограничений: `S` используется и при отсутствующей или неограниченной
квоте. Без обоих значений лимит CPU неизвестен. В прежних записях контекста для
использования `S` нужна известная квота. Процент памяти вычисляется по записанному
конечному лимиту. Учитываются доступные ограничения предков; ограничения выше
видимой иерархии недоступны. Лимиты относятся к выбранной группе, а не к дочерней
группе сборщика или PostgreSQL. Смена или пересоздание
группы разрывает расчёт приростов и историю.

Источники: [чтение иерархии](../crates/kronika-source-os/src/cgroup.rs),
[контекст](../crates/kronika-registry/src/codec/os_cgroup_context.rs),
[ресурсные показатели](../crates/kronika-query/src/hour/lanes.rs),
[отображение и история](../bins/kronika-web/ui/src/system-view.tsx).

<a id="controller-metrics"></a>

### Показатели контроллеров

Контроллер учитывает расход своего ресурса всей группой. В формулах `used_cores` — занятые ядра, `effective_capacity` — лимит CPU в ядрах из раздела выше, `current` — текущее значение соответствующего контроллера, `max` — его локальный предел. `effective_memory_max` — записанный лимит памяти с учётом доступных ограничений родительских групп.

| Метрика или записанное поле | Расчёт/источник | Единица |
|---|---|---|
| CPU used / user / system | `R(usage_usec) / 10⁶`, `R(user_usec) / 10⁶`, `R(system_usec) / 10⁶` | Занятые ядра; `cpu.stat` |
...
| CPU share | `100 × used_cores / effective_capacity` | Доля записанного лимита CPU, %; без известного лимита недоступна |
| CPU quota / period | `quota_usec`, `period_usec`; лимит CPU в ядрах равен `Q/P` при положительных исходных значениях | Локальный предел CPU; квота `−1` означает отсутствие ограничения |
...
| CPU / memory / I/O PSI | `100 × R(some_total) / 10⁶` для ожидания ресурсов выбранной cgroup | Доля интервала ожидания, % |
| Memory current | `memory.current` | Байты |
| Memory share | `100 × current / effective_memory_max` | Доля положительного записанного лимита памяти, % |
| Local memory max | `memory.max` | Байты; в новой структуре `max_unlimited` отдельно обозначает неограниченную память, отсутствие данных остаётся неизвестным |
...
| Memory events | Накопленные `low_events`, `high_events`, `max_events`, `oom_events`, `oom_kill` | Накопленные события; график OOM использует `R(oom_kill)` завершений за секунду |
| I/O read/write | `R(rbytes)`, `R(wbytes)` из `io.stat` | Байты в секунду для пары cgroup/устройство |
...
| Хранилище машины | Занятость самого занятого устройства, % | Очередь того же устройства | Недоступно |
| Сеть машины/пространства имён | RX и TX, байты/с | Потери с учётом FIFO за секунду | Ошибки RX + TX за секунду |
| CPU cgroup | Доля лимита CPU, %; без него — занятые ядра | Ограничение квотой Throttled, % и CPU PSI, % | Недоступно |
