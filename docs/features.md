...
| Row, Inspector Detail / Chart | Select identity and show recorded facts or selected history metric. Related tabs depend on the row type. |
...
| Inspector divider | Resize with the mouse. Keyboard arrows adjust width, Home/End choose limits. Narrow layouts use an overlay or bottom sheet. |
...
| Activity entity label | Apply the corresponding table’s entity/group filter where supported and move to its busiest interval when absent at the cursor. Cgroup labels have no table-filter action. |
| `?`, field help, Esc | Open general help / metric definition or close an open panel or selection. |
...
| Input | Meaning |
| --- | --- |
| Collector `--pg-dsn` or `KRONIKA_PG_DSN` | One connection selects the PostgreSQL server to collect. |
...
| Web `--sources` or `KRONIKA_WEB_SOURCES` (default: `all`) | Source names `none`, `os`, `postgresql`, `all`, or the legacy unsigned catalog bitset: `0` neither configured flag, `1` OS, `2` PostgreSQL, `3` both. It labels configured sources. It does not filter stored data, control collection, hide tabs or calculate Health. |
...
`POST /mcp` exposes fourteen stored-data tools over stateless Streamable HTTP. Each tool returns structured JSON. The endpoint rejects `Origin` and query strings.
...
The four installed programs are `kronika-collector`, `kronika-web`, `kronika-dump` and `kronika-report`. CLI [slice](../bins/kronika-dump/README.md) creates a recording range, and [report](../bins/kronika-report/README.md) converts it to HTML.
