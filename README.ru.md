...
Один процесс сборщика сохраняет данные одного сервера PostgreSQL и его
доступных баз в отдельный каталог хранения. Выберите `local` для Linux и
PostgreSQL той же VM или pod; выберите `postgresql` для удалённого сервера или когда OS не нужна.
