...
Версия **1.1.0 ещё не выпущена**. [Соберите её из этих исходников](docs/build.ru.md)
или [установите](INSTALL.ru.md) соответствующий [архив для разработки](docs/releases.ru.md#development-builds).
Для команд ниже `kronika-collector --version` должен показывать `1.1.0`.
...
указав его DSN и отдельный каталог хранения. Для всех процессов используется
один и тот же бинарник; каждый процесс собирает метрики доступных баз своего сервера. См.
