...
use finalize::write_final_dictionary;

/// Bounds temporary Arrow arrays while preserving deterministic page boundaries.
/// This row count also feeds the conservative encoded-size bound.
...
pub(crate) fn encode_final_entries_to<'a>(
    ...
    let entries = entries.into_iter().collect::<Vec<_>>();
    validate_final_entries(&entries)?;
    let (mut strings, mut blobs): (Vec<_>, Vec<_>) = entries
        .into_iter()
...
fn encode_strings(
    ...
    let schema = dictionary_schema(DICT_STRINGS_TYPE_ID)?;
...
fn encode_blobs(
    ...
    let schema = dictionary_schema(DICT_BLOBS_TYPE_ID)?;
