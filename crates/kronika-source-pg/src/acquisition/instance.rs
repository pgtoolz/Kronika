...
use crate::Pool;
use crate::query::{self, BatchWrite};
...
use super::{PgBatch, PgCollector, PgObservation, PgWarning};
...
impl PgCollector {
    ...
    pub(super) async fn collect_activity<E>(
        ...
        if probe.full_visibility {
            ...
            (pass.observe)(PgObservation::Warning(PgWarning::StatsVisibilityRequired {
                database: probe.database.clone(),
            }));
        }
        Ok(true)
    }
