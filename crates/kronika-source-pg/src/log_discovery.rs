...
use crate::acquisition::{ConnectionObservation, PgObservation, QueryObservation, QueryOutcome};
...
// SQL text or its diagnostic identities.
...
impl ConnectionTarget {
    ...
    ///
    /// # Errors
    /// Returns an opaque error for a malformed DSN or inconsistent endpoints.
    pub fn parse(raw: &str, source_index: usize) -> Result<Self, InvalidConnection> {
        let config = Config::from_str(raw).map_err(|_error| InvalidConnection)?;
        validate_endpoints(&config)?;
        let label = connection_label(&config, config.get_user(), source_index);
        Ok(Self {
            config,
            label,
            source_index,
    ...
    /// Credential-safe endpoint label, excluding passwords and database names.
    #[must_use]
    pub fn label(&self) -> &str {
        &self.label
    }

    /// Configured database name, or a placeholder until the server resolves it.
    #[must_use]
    ...
    fn label_for_user(&self, user: &str) -> String {
        connection_label(&self.config, Some(user), self.source_index)
    }

    /// Caller-provided source identity used for diagnostics.
    #[must_use]
...
pub struct PostgresServer<T> {
    ...
    /// Server timezone interpreted by the caller's resolver.
    pub log_timezone: T,
...
///
/// The timezone resolver runs within the measured log-facts query, before
/// identity lookup. Invalid timezone names therefore fail that query without
/// changing cached identity or reporting a successful metadata refresh. The
/// source library has no dependency on the caller's log parser or timezone type.
///
/// # Errors
///
...
pub async fn postgres<T>(
    target: &ConnectionTarget,
    transport: &Transport,
    cached_system_identifier: Option<u64>,
    discover_paths: bool,
    resolve_timezone: impl FnOnce(&str) -> Result<T>,
        ...
        match connection::connect_monitoring(&target.config, transport).await {
            ...
            Err(failure) => {
                ...
                let (timeout, message, error) = match failure.error {
                    ...
                    ConnectionFailure::Timeout(error) => (
                        true,
                        format!(
                            "{context} timed out after {} seconds",
                            CONNECT_TIMEOUT.as_secs()
                        ),
                        anyhow::Error::new(error).context(format!("{context} timed out")),
                    ),
                };
    ...
    let facts = query::timeout(
        session,
        QUERY_TIMEOUT,
        read_log_facts(session, &mut facts_stats, discover_paths, resolve_timezone),
    )
    .await;
...
async fn read_log_facts<T>(
    ...
    let line_prefix = row
        ...
        .to_owned();
    let log_timezone = resolve_timezone(
