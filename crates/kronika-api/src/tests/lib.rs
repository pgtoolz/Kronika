...
use kronika_query::{
    ...
    IndexRequest, Order, QueryRequest, RelationGroup, SegmentRequest, StatementScope, Window,
};
...
fn relation_hour_series_accepts_only_one_exact_aggregate_scope() {
    ...
    assert_eq!(series.group, Some(RelationGroup::Schema));
    ...
    let series = hour.series.expect("relation series");
    assert_eq!(series.group, Some(RelationGroup::Tablespace));
...
fn snapshot_paging_inputs_enable_one_bounded_page() {
    ...
    assert_eq!(grouped.group, Some(RelationGroup::Schema));
